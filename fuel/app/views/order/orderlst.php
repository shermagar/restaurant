  ...
  <td>Rs:<input type="text" id="total" class="amount" readonly value="" name="amount" ></td>
   <input type="hidden" id="status<?=$product->id?>"  value="0" name="status" >
  ...
  $(".check:checked").each(function(){
   ...
   $.ajax({
      ...
      data: {'check':checkval,'quantity':qty,'order_id':ord,'status':status },

      dataType:'html',
      cache:false,
      success:function(){
        location.href = '<?php echo uri::base().'order/orderlst'?>';
              }
